...
type Options struct {
	...
	// Security configures CEF-backend hardening options. The fields
	// are no-ops on non-CEF builds so apps that set them still
	// compile when linked against the WebKit/GTK4 backend.
	Security SecurityOptions
}

// SecurityOptions configures CEF-specific runtime hardening. All
// fields are opt-in (defaults preserve current behavior).
type SecurityOptions struct {
	// AllowedMethods, when non-empty, restricts `Call.ByName(...)`
	// invocations to the listed fully-qualified method names (e.g.
	// "main.Greeter.Hello"). Calls not in the list are rejected at
	// the JS wrapper *before* the native V8 binding is invoked, so
	// they never reach the Go MessageProcessor. Empty (default)
	// means "no restriction" — all calls are forwarded, matching
	// the previous behavior.
	//
	// Method names are matched case-sensitively against the runtime's
	// request payload's `methodName` field. They do NOT restrict the
	// built-in subsystems (Window, Events, Application, System,
	// Screens, Clipboard, Browser, Dialogs, Call) — those are always
	// allowed because the runtime uses them internally.
	AllowedMethods []string

	// EnforceCSRFNonce, when true, requires every CEF-mediated
	// runtime call to carry a per-navigation nonce as a second
	// argument to `window.wails.invoke` /
	// `window.wails.invokeAsync`. The nonce is generated by Go on
	// `OnDocumentAvailableInMainFrame` and exposed via
	// `window._wails.cefNonce`. Page scripts can read it, but a
	// script that attempts to call wails.invoke without the nonce
	// is silently rejected.
	//
	// The runtime bundle (internal/assetserver/bundledassets/runtime.js)
	// does not pass a nonce — when this option is enabled the
	// runtime will fail to call services unless the Go side
	// auto-injects the nonce into every runtime call. We do that
	// by having Go wrap the JS `wails.invoke` call to prepend the
	// nonce from the wrapper itself rather than passing it
	// explicitly. See cef_js_shim.js for the implementation.
